...
**WorkWell Measure Studio** lets occupational-health and compliance teams take regulation text (e.g., OSHA 29 CFR 1910.95 — annual audiograms for noise-exposed workers), convert it to executable CQL, run it against employee health data, and generate **worklist cases** — concrete follow-up actions for non-compliant employees with full "why flagged" evidence and audit trail.

Replaces spreadsheets and manual chart review with policy-to-CQL → run → operationalize pipeline. MVP = three layers:
...
Occupational-health and compliance teams own medical surveillance requirements varying by site, job role, and exposure risk:
...
- Requirements interpreted inconsistently across teams.
- "Due vs overdue" logic varies by person.
- Work reactive — someone checks spreadsheet, chases people.
- During OSHA audit, defending determinations is hard with no traceable evidence trail.
...
This is exactly the gap eCQMs fill in healthcare quality reporting. WorkWell applies same pattern to occupational-health compliance.

---

## 3. Vocabulary & Key Concepts

Glossary for the project. These terms drive architectural choices.
...
### 3.5 Value Set
Curated list of clinical codes all meaning the same thing for measure purposes. Identified by OID like `urn:oid:2.16.840.1.113883.3.464.1003.118.11.1`. Example: "Audiogram Procedures" value set bundles several LOINC codes all representing audiometry tests.
...
Taxonomy debate from meeting notes: one broad measure with parameters vs many narrow specific ones? For "Annual Physical by Role," lumpers want one measure with role parameter; splitters want six role-specific measures. **Recommendation for MVP:** start as splitter (clearer demos, cleaner CQL), refactor later if duplication hurts.

### 3.8 "Vectorization of value sets"
From meeting notes — converting code lists into vector embeddings so AI assistant can suggest right value set when author types measure name. Phase 4 stretch; aligns with prior summer's RAG work.
...
Single most important correctness property. Cases keyed by `(employee_id, measure_version_id, evaluation_period)`. Running same scope twice produces zero duplicate cases. Without this, every rerun creates noise and worklist becomes useless.
...
Policy-to-CQL authoring and execution workflow that:
...
3. Operationalizes outcomes as actionable worklist cases with explainability and audit.
...
CRUD over `Measure` and `MeasureVersion`. Lifecycle state machine. Version cloning ("v1.1 → v1.2 with change summary").
...
**Buffer policy:** if Phase 2 slips a week (likely — see Risks), Phase 3 slips a week. Don't compress Phase 2 to make up time; data model built there is foundation for everything after.
...
**Goal:** clean, repeatable demo with credible auditability and open-source-ready repo.
...
- GitHub Actions workflow stub: just `echo hello` on PR
...
- Note: Synthea occupational health modules don't ship by default — custom OH module is Phase 0 stretch. For Week 1, use default population so data flows.
...
- NextAuth in dev mode (no real auth, just session)
...
**[0:00] Programs Overview** — "Here's what an occupational health team sees Monday morning. Four programs, compliance rates, last run." Click TB Surveillance → Program Detail with top drivers.

**[0:45] Worklist** — "12 cases at Plant A clinic are overdue for TB screening. Here's the queue." Filter by site → click first case.

**[1:30] Case Detail with Why Flagged** — "Maria Rodriguez, Nurse, Plant A clinic. Last TB screening was 412 days ago. Here's the evidence — actual data points the rule used." Show structured evidence.

**[2:15] AI Explanation** — Click "Plain English" → AI summary appears: "Maria is overdue because her last TB screening (Mar 2025) is more than 365 days old. She has no active waiver and is in a high-risk role." Note: generated *from* structured evidence, not separate inference.

**[2:45] Take Action** — Click "Send outreach reminder" → templated message; delivery status updates.

**[3:00] Behind the scenes — Measure Studio** — "Where does this rule come from? Here's the measure that flagged her." Open Annual TB Screening v1.3 → Spec → CQL.
...
**[4:15] AI Draft Spec** — "Here's how we'd add a new measure quickly." Paste OSHA paragraph → click AI Draft Spec → draft populates → "Human reviews and decides."

**[4:45] MCP integration** — Pivot to Claude Desktop: "Agent talking to system." Run: "Show me overdue cases at Plant A and explain the top three." Claude returns answer using MCP tools.
...
Didn't have clear answers in existing docs; may surface during Week 0 discussions with Doug.
...
*End of plan. Update this doc as decisions are made — it's the working source of truth for the project.*