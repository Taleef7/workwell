...
| 11 | **Authoritative subject resolver + PHI-enabled attributed-list import** (ADR-082 d4) | **Required** — the import is refused outright on a live-directory deployment today, because that directory is a worker-local last-known registry that FABRICATES a profile for any `wc|` id, so matching against it would be silently incomplete rather than unavailable. A real attribution file must not reach Neon before #267/#265/#264 either | #557 |
| 12 | **Per-run report ARCHIVE for the attributed list** (ADR-082 consequences) | **Required for the ACO's ~10-year audit window** — with a 400-day retention window the latest nightly is never exposed, so a live report renders today; what is lost is REPRODUCIBILITY, because by December the runs behind March's filing are compacted and ADR-077 refuses to re-render them. Design: `subject_list_reports` (id, list_id, generated_by, generated_at, run_ids_json, summary_json, payload_hash, payload_size_bytes) + `subject_list_report_rows` (report_id, measure_id, rows_json) so a 50,000-member CSV streams per measure; `POST …/reports` snapshots the live computation and it is served from the archive with no compaction gate. **Interim stopgap (owner-ops, no code):** every report actually handed to the ACO is retained as a dated file in the R2 evidence bucket, and its CSV rows carry the list revision and run ids, so a December question about March has an artifact to point at. It is a manual stopgap, not a system of record | #557 |
