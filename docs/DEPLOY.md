...
|---|---|
| unset | the script **warns and leaves the capped codes exactly as upstream shipped them**; `manifest.terminology.truncated` survives and official routing keeps refusing at construction. Deploy still succeeds — the safe direction, and the same one a missing sidecar takes |
| set | the shortfall is completed, `truncated` empties, and `manifest.terminology.completion` records the release the codes came from |
