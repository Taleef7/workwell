...
| `WORKWELL_WEBCHART_PATIENT_SEARCH` | Backend | Optional raw FHIR query for the population-listing request, for servers that 403 a bare `GET /Patient` (teatea does — verified 2026-07-23). Default fallback (auto-engaged on a 400/403 first page) is `birthdate=gt1900-01-01`, which enumerates the whole population via an accepted indexed search; set this to pin a different accepted query. |
