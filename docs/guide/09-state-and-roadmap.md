...
| Measure page read path, the fix (code 2026-09-15, sandbox numbers pending the deploy) | `programRiskOutlook` moves off the measure's whole retained history onto the winning run, memoized under the winners' `runKey` with `today`/`horizonDays` applied per request; the repeat-non-complier streak retired (ADR-081); the page paints per panel instead of gating on all four reads | **BEFORE: `risk-outlook` 504 cold, 8.2 s warm; `/programs/cms125` rendered no heading within 20 s on one of two attempts** (the Maui e2e suite's only flake, 2026-09-15). The AFTER numbers — `risk-outlook` cold and warm, and the measure page's first render — are measured on the next deploy **outside the nightly recompute window** and recorded in `docs/JOURNAL.md`, never predicted here |
