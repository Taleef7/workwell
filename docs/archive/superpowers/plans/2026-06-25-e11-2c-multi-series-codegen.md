...
- [ ] **Step 1: Roster assertion.** Add a `deriveCell` test for repointed Hep B: evidence with `Dose Count` = 2 + `Outcome Status` COMPLIANT → `{ status: "COMPLIANT", method: "2 valid dose(s)" }`; `Dose Count` = 1 + canonical MISSING_DATA → `{ status: "IN_PROGRESS", method: "1 of 2 doses on file" }`; `Dose Count` = 0 → `MISSING_DATA "No doses on file"`. (No `deriveCell` code change expected — this pins the behavior.)
  > **Display nuance (from the PR-1 final review — display-only, never affects `Outcome Status`):** the roster method string uses the **union** `Dose Count` for COMPLIANT (`N valid dose(s)`) and the top-level `series.requiredDoses` (2) for the IN_PROGRESS denominator. For an employee mid-**traditional-3** series, "1 of 2 doses on file" under-reads that brand's true 3-dose target. This is accepted (the canonical bucket is CQL-authoritative; we deliberately kept `requiredDoses=2` per the user decision to avoid making `deriveCell` alternative-aware). Call it out in the JOURNAL/MEASURES note (Task 8) so the approximation is documented, not silent. Only revisit (make `deriveCell` alternative-aware) if a future ask needs an exact per-brand denominator.
