...
- `engine`: Spring-free measure-evaluation seam. `engine.port` declares the input ports (`PatientDataProvider`, `EmployeeDirectory`, `MeasureDefinitionProvider`, `EvaluationConfigProvider`); `engine.model` holds `MeasureDefinition`; `engine.synthetic` provides the default demo adapters (the synthetic FHIR bundle, employee catalog, measure bindings, and compliance rates). `CqlEvaluationService` is constructed from these ports, so real EHR/FHIR data adapters can be added later without touching the core. The synthetic adapters are the default beans; the live demo runs on them (ADR-005, ADR-004 unaffected). A `EngineNoSpringContextTest` proves the core evaluates with no `ApplicationContext`.
