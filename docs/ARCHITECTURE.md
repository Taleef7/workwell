...
- Tenants (#185 / E13 PR-1): `GET /api/tenants` → `{ id, name }[]` — the WebChart systems for the multi-tenant selector. Authenticated under the catch-all `GET /api/**` (all roles, read-only). Read-time; no schema.
