...
| Patient-level report | HL7 QRDA Category I | `GET /api/runs/{id}/qrda1` (one CDA per subject) (M-B) | **Not conformant — well-formed and structurally recognisable only** | Measured against the official CMS 2026 QRDA Cat I Schematron, so the gap is a list rather than a hedge: required `author`, `custodian` (CCN), `legalAuthenticator` and the CMS EHR Certification ID `participant` are **absent**; `<addr>` is a hard-error `1..*` with **no nullFlavor escape**, so a patient without an address cannot produce a valid document (a data-ingest prerequisite, not formatting); `administrativeGenderCode nullFlavor="NI"` is Schematron-clean but IG-wrong (`OTH`/`UNK`/`ASKU`). **The Patient Data section is EMPTY**: no QDM entries, so the artifact reports what we computed and cannot be used to RECALCULATE it. Population membership is evidence-first. QRDA I **import** does not exist. Cypress **CVU+** has not run. **Open:** CMS Cat I may be Hospital-QR-only, with ECs/MIPS submitting Cat III — unverified, and distinct from the ONC §170.315(c) export path. (ADR-049) |
