...
| Official execution over REAL EHR data | Official CMS125 QI-Core v1.0.000 artifact vs WorkWell's authored implementation | `devdb-official-eval.test.ts` runs MIE's WebChart dev-DB sample (56 patients) through the live ingress path and compares official vs authored outcomes per subject | **Executed / authored-parity only — NOT verification** | Official CMS125 agrees with authored on all 56 (52 MISSING_DATA, 4 OVERDUE) after `us-core-sex` was mapped from WebChart's `patients.sex`; official CMS122 and authored are both blind on this seed (no Conditions). **Read the limits:** the oracle is our own authored engine, not an external expected answer, so agreement is evidence the flip is safe *for this data* — not that either engine is correct. 52 of 56 outcomes are MISSING_DATA, so only **4 subjects carry discriminating signal**. Nothing is routed (`WORKWELL_OFFICIAL_MEASURES` unset). Cypress CVU+ remains the verification bar and has not run (ADR-042) |
