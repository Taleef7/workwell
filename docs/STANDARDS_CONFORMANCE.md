...
| Patient-level report | HL7 QRDA Category I | `GET /api/runs/{id}/qrda1` (one CDA per subject) (M-B) | **Stub** | Well-formed + structurally representative; **not** IG/Schematron-validated and **not** CVU+-validated. Population membership is evidence-first (official `populationResults` when present, never the workflow status). **The Patient Data section is deliberately EMPTY** (`nullFlavor="NI"`, with a note in the document): the QDM entries a receiving engine would need to RECALCULATE the measure are not exported, so this artifact reports what we computed and cannot be used to reproduce it. QRDA I **import** does not exist. (ADR-049) |
