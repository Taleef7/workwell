...
**H1 status:** Java + `backend-ts` at parity on Phase 1 + Phase 2 root fix + A worklist default + M6 (eval-date + the window fix). **One item remains:** (D) a one-time cleanup of the ~4,703 live stale-period open cases — a **production-data migration** (Taleef authorized it explicitly, overriding the CLAUDE.md schema-ownership rule for this case); to be designed audited + idempotent and bundled into the H1 PR. Remaining #150 after H1: H4 (verify — likely covered by C4), M1, M5, M8, M9, M10, M13.
