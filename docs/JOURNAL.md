...
- **P1 #1 — worklist `MAX` poisoned by closed stale rows.** The `current`-cycle subquery took `MAX(evaluation_period)` over **all** statuses; after V022 closes a stale raw-date row whose period (`2026-06-15`) is lexically *later* than the new cycle anchor (`2026-01-01`), that closed row would win the MAX and **hide the current cycle's open cases**. Fixed by taking the MAX over **open cases only** (`closed_at IS NULL`) in Java `CaseFlowService` + both `backend-ts` stores; new closed-row regression test.
...
**Validation:** Java compiles clean; `CqlEvaluationServiceTest` (incl. M6), `NightlyRunIdempotencyIntegrationTest`, `CaseWorklistPeriodIntegrationTest`, `ProgramRollupRerunIntegrationTest`, and `ScopedRunFailureIntegrationTest` all green against real Postgres (Docker brought up locally). `backend-ts` **377 tests — 376 pass / 0 fail** (1 PG suite skipped); typecheck clean. V022 apply-validated by the Testcontainers integration tests + read-only-validated against live Neon earlier (5,019 open / all stale / 0 anchored).

**H1 status: COMPLETE** (Phase 1 + Phase 2 [re-layered] + A worklist default + M6 + D, Java ↔ `backend-ts` at parity; both Codex P1s resolved). Remaining #150 after H1: H4 (verify — likely covered by C4), M1, M5, M8, M9, M10, M13.
