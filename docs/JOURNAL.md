...
### Then it was wrong again, and the tool was why

Codex found `attachValueSet` and `detachValueSet` missing from the "complete" list. They are flipped
now — everything the event needs is known before the link is written — but the interesting part is
the cause: the sweep matched a **verb whitelist** (`create|update|set|insert|…`), so
`valueSets.link(...)` and `unlink(...)` never matched anything at all.
...
states the rule per WRITE rather than per function, and says outright that it is not a complete
inventory and that #598 does not close on it.

Twice in one evening an "authoritative" enumeration in an always-loaded file turned out to be an
artifact of the matcher that produced it. The lesson that stuck is narrower than "check your work": a
sweep is worth exactly its matcher, and a list in a document that a session must not contradict
should either be complete or say loudly that it is not.
...
## 2026-09-10 (the practice's feedback call) — what shipped in 48 hours, and the five asks that are one missing input

*Recorded 2026-09-15. Three of this call's asks reached the 09-12 entry; the remaining cluster — which
is the larger half — did not, and none of it was filed. Roles, not names.*

**What the call asked for and got, inside two days.** The PCP filter. The patient-first work list, one
row per person with every open gap on it and no duplicate rows. Going straight from the list to the
patient. Assignment that actually works (#551 — before it, the select was inert). Staff-to-provider
panels with bulk reassignment (#555, ADR-080). The primary-insurance filter, as a *set*, because the
typology is hierarchical and a single-valued Medicare filter would have withheld 2,900 Medicare
Advantage patients under a heading claiming to contain them. Site, PCP and insurance running through
one predicate rather than four copies of it. The timeline collapsed. And the answer to the 14,000
missing-data question: those patients were outside the measures' populations and were being counted as
gaps, which ADR-079 fixed — **moving the reported rates by 8 to 54 points**.

**Nobody has told them the numbers on their screen changed.** CMS125 18.0 → 72.1%, CMS165 20.5 →
62.4%, CMS130 19.5 → 43.3%, CMS2 60.9 → 68.7%, CMS137 0.4 → 13.5%, and CMS122 7.4 → 72.4% — that last
one is an **inverse** measure (poor glycemic control), so for it a higher number is worse, and it is
the one most likely to be misread as a collapse in quality. A quality lead who looked before and after,
with no message in between, has every reason to distrust both readings. It is the first item on the
owner's email list.

**The biggest cluster of unmet asks is five asks that collapse into one missing input.** Seeing a
patient's upcoming appointments; branching the work list between "prep for a visit already booked" and
"reach out to someone with none"; making "prep" create an encounter; scheduling onto the provider's own
calendar; adding an encounter from a card. Every one of them needs **appointment data out of
WebChart**, and there was no issue and no roadmap line for it. Now ROADMAP §7.12, owed by MIE.

**Two more that were unfiled and are now named.** Per-staff card configuration — line staff see fewer
cards, supervisors see everything (§7 and its own issue). And the outreach attempt ladder, text then
email then phone, with letters generated by document type: the channels exist and are simulated, but
there is no policy layer and no document generation.

**One ask cannot be built as stated, and that has to be said rather than quietly dropped.** "Upload
evidence with a document type that fulfils the measure" collides directly with CQL-is-truth (LOCKED
§4A.3): an upload cannot satisfy a measure, because the measure reads coded data and re-evaluates. The
honest forms are a coded result written back into the chart, or MIE's order/document-code mapping — both
of which are §7.13 and §7.1, both owed by MIE. Saying so is the deliverable here.

**The smallest high-value miss, and it is a screen.** The ask was framed on the measure roster —
filter for a provider, a measure and an insurance, then assign that report. `/compliance` has neither a
payer control nor an assign affordance; `/worklist` has both. So the capability exists on the wrong
page, which from the other side of the call is indistinguishable from not existing. It is the next
small customer-facing PR, and the assign semantics need defining rather than assuming: roster cells are
outcome references, not case ids, so "assign selected" can only mean *that measure's active case per
patient*, and only with exactly one measure in scope.

**A design decision from the call that does not do what it is meant to do.** Stamping Z13.31 on the
assessment for a positive depression screen solves a real patient-harm problem and is a sensible thing
to do — and it earns **no CMS2 credit**, because that measure's numerator wants a coded follow-up
(a procedure, service request or medication request), not a diagnosis code. Also, "3 or higher" is the
PHQ-2 cutoff, not the PHQ-9's. Raise both; build neither.

**Facts from the call that had no home and now have one.** MIE's November prior-authorization target.
A "certification requirement that's coming out" — which needs checking against ADR-058's single reopen
condition, since the whole no-certification posture rests on MIE not treating engine certification as a
business goal. The proposed rule's July 14 date. And MIE's ticket-routing protocol. The developer's
eSIM/WebChart access was promised on this call and had not arrived by 2026-09-15; it gates #556's
verification and the specification of everything above (§7.14).
...
## 2026-09-09 (the ACO working session) — the measure set is confirmed, and the one thing they actually asked for
...
(479 HWR, 484 MCC, 321 CAHPS) are claims- or survey-calculated, computed *for* them rather than *by*
them, and are correctly outside the engine's scope. ROADMAP §7.7 is discharged on this.

**The reporting answers, which had been open since 2026-08-30 as §7.5.** Reporting is at the ACO level
under APP Plus; the practice becomes a MIPS APM through it; the workflows are eCQM-only; PY2027 is
reported in 2028. And the mechanic that turns out to be a product requirement rather than a detail:
**the mechanism is chosen per measure, and both participants must agree.** The ACO computes each
measure under every mechanism available to it, compares against the benchmarks, and takes the best. So
our numbers have to be available *early* and in a shape that supports a per-measure comparison. That is
now §7.5 rather than a thing I remembered.

**What they actually asked for is one feature, and it is not built.** Upload the attributed patient
list; run the eCQMs over exactly that subset; hand back numerator, denominator and exclusions, plus
patient-level evidence carrying the result **and the date**. That is #557, and a repository-wide search
for `subject_list` returns nothing. It is blocked on owner-written DDL and on four inputs the ACO has
not sent (identifier format, cadence, how non-matches are handled, whether Medicare Advantage is in or
out), and the 2026-09-14 plan ships it as two PRs. Their own prioritised report list was promised on
the call and had not arrived by 2026-09-15.

**Several things they raised are already true, which is worth recording as much as the gaps.** 305's
removal is *proposed* and not final (the final rule lands ~Nov 2026). Outside PDFs do not count toward
a measure, and the roster already says so with a "check for outside records" next action. Hospice,
palliative and frailty exclusions are in the corpus. QRDA I and III exist as the interoperability
bridge. A Medicare-primary, PCP-scoped gap list is what #551's payer multi-select and #555's panels
produce. And "never auto-apply external data" became #557's design rule rather than an aspiration: a
non-matching member lands in a review queue, and a manual resolution is a new list revision, never an
update.

**Two contradictions that need an owner, not a commit.**

1. **400-day retention against a roughly ten-year audit expectation.** They said the audit window out
   loud. Maui ships `WORKWELL_OUTCOME_RETENTION_DAYS=400`; ADR-073's keep-set preserves the newest
   usable row per (subject, measure, period) forever, so the *live* numbers are safe — but ADR-077
   makes every report surface answer **409 `run_compacted`** once its run predates a compaction cutoff,
   and #557's report package inherits that refusal. A report filed in March is therefore not
   re-renderable in December: not wrong, *refused*, which is the right behaviour and the wrong
   capability. Now `PRODUCTION_READINESS` item 9, with the archive design recorded and a manual
   stopgap (retain every handed-over report as a dated file in the R2 evidence bucket).
2. **"All Medicare" is typology codes 1 and 11, and MSSP attribution is traditional Medicare.** A
   quality lead will read that control as "the ACO population" and receive 2,900 Medicare Advantage
   patients with it. The filter is correct and the *label* is the problem. Either the ACO answers input
   4 or the control gets relabelled until they do.

**And one absence worth noting.** CEHRT, FHIR and MeasureReport were never mentioned. There is no
customer pull for FHIR-shaped reporting — which does not make the FHIR column the wrong verification
bar (locked decision 2), but it does mean no roadmap item may be justified by customer demand for it.

**Owed by whom, so the silence is visible:** the four #557 inputs, the prioritised report list, the
report format (QRDA-I or CSV/JSON — both exist), and whether the practice's separate MIPS submission is
in our scope, are all theirs. The ACO's own structure is unclear enough to matter — who submits, and
who is benchmarked against whom — and is recorded pseudonymously as §7.17. One discrepancy stays open
on purpose: CMS-1848-P proposes removing Quality IDs **305 and 493**, and only 305 was named as at
risk; 493 is not in the six, so nothing depends on it, and neither source is silently trusted (§7.16).
