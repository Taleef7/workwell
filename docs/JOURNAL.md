...
**H1 status:** Java + `backend-ts` are now at parity (Phase 1 + Phase 2 root fix + A worklist default + the M6 eval-date half, on both stacks). **Remaining for H1:** (D) a one-time cleanup of the live stale-period cases — a **production-data migration**, so it stops for explicit sign-off before any `V0xx` file is written (CLAUDE.md schema-ownership rule); and the **`why_flagged` derivation half of M6** (the 439-vs-60 day-math: a builder fix on both stacks, an open item on each — not a parity gap). Then the H1 PR. Remaining #150 after H1: H4 (verify — likely covered by C4), M1, M5, M8, M9, M10, M13.
