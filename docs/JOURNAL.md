...
**H1 status: COMPLETE** (Phase 1 + Phase 2 root fix + A worklist default + M6 + D, Java ↔ `backend-ts` at parity). Ready for the H1 PR (Java + `backend-ts` + V022) → Codex. Remaining #150 after H1: H4 (verify — likely covered by C4), M1, M5, M8, M9, M10, M13.
