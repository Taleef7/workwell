...
**Validation:** Java compiles clean; `CqlEvaluationServiceTest` (incl. M6), `NightlyRunIdempotencyIntegrationTest`, `CaseWorklistPeriodIntegrationTest` (incl. P1 stale-row + P2 terminal-tab guards), `MeasureImpactPreviewIntegrationTest`, `ProgramRollupRerunIntegrationTest`, and `ScopedRunFailureIntegrationTest` all green against real Postgres (Docker up locally). `backend-ts` **418 tests / 0 fail** (1 PG suite skipped); typecheck clean. V022 apply-validated by the Testcontainers integration tests + read-only-validated against live Neon earlier (5,019 open / all stale / 0 anchored).

**H1 status: COMPLETE** (Phase 1 + Phase 2 [re-layered] + A worklist default + M6 + D, Java ↔ `backend-ts` at parity; both Codex P1s + both P2s resolved). Remaining #150 after H1: H4 (verify — likely covered by C4), M1, M5, M8, M9, M10, M13.
