...
- **Not point-of-care alerting yet:** WorkWell includes a standards-conformant Clinical Decision Support (CDS) Hooks service capable of delivering care-gap cards into a clinical workflow. However, no client system—including WebChart—currently invokes it. Alerting today remains WorkWell-screens-only via our dashboards and worklists.
...
- **Runs CMS's own published logic:** Many quality systems rewrite official electronic clinical quality measures (eCQMs) into proprietary rules, risking logic discrepancies. For official Centers for Medicare & Medicaid Services (CMS) measures, WorkWell executes CMS's published measure files directly, verified against the measure authors' own test cases.
- **Evidence retained per patient per measure:** Rather than producing an opaque yes/no score, WorkWell retains the result of every named rule in the measure logic for every patient and measure. Quality teams and auditors can review the exact clinical rationale behind every status.
