...
- [x] Live probe done 2026-07-13: R4 4.0.1 + private_key_jwt/RS384 + system/*.read confirmed; grant list advertises authorization_code only. Recorded in INTEGRATION_RESEARCH §1.1a (PR #286).
- [x] Attempted 2026-07-13: NO registration endpoint openly enabled on the sandbox (none advertised; /register paths fall through to the login UI) → token exchange unreachable without MIE-side registration. Sharpened #254 ask recorded (register a WorkWell backend-services client / enable RFC 7591).
