...
export class PgCaseEventStore implements CaseEventStore {
  ...
  async insertAction(input: InsertActionInput): Promise<void> {
    await this.pool.query(
      `INSERT INTO ${SPIKE_SCHEMA}.case_actions (case_id, action_type, payload_json, performed_by, performed_at)
       VALUES ($1, $2, $3::jsonb, $4, $5)`,
      [input.caseId, input.actionType, JSON.stringify(input.payload), input.actor, new Date().toISOString()],
    );
  }
