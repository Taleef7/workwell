...
- **The incremental-evaluation cache learns about official routing (PR #348; ADR-040).** Closes a
  ...
  flip-off, and re-vendor-while-routed all invalidate reuse by construction. Inert on every environment
  today (both flags unset ⇒ byte-identical run loop); no schema change (`logic_version` is already TEXT).
...
- **WebChart-convergence roadmap (epics E10–E16), all merged + deployed.** Highlights:
  - **E10/E11 — roster-centric compliance + rule builder:** an "Individual Compliance Status" roster (`/compliance`), a PERMANENT/RECURRING taxonomy + 3 series-completion immunization measures (14 runnable), CQL-canonical rule→codegen with a Studio Rule Builder tab, and risk-group **Segments** (owner-gated cohorts gating case creation/display, never compliance).
  - **E13 — multi-tenant (multi-system) rollup:** a tenant/system dimension above enterprise→location→provider→patient, two synthetic WebChart systems (`twh`, `ihn`) + a population-scale `mhn` tenant (~120k subjects, SQL-aggregated), a reconciling "All Systems" root, `?tenant=` scoping everywhere, and a 24-hour scheduled `ALL_PROGRAMS` recompute.
  - **E14 — standards fidelity:** `GET /api/measures/:id/fidelity(+/diff)` diffing WorkWell's authored eCQM against the official CMS122v14 spec (COVERED/SIMPLIFIED/OMITTED + criteria-impact outcome diff; structural-first).
  - **E15 — cross-system identity:** a read-time person layer (`/people`) resolving one person across systems (DUPLICATE candidates, mobility timeline) + an owner-gated `person_links` reconcile write path (audited CONFIRM/UNLINK + merge-picker).
  - **E16 — quality over time:** a materialized `quality_snapshots` aggregate (per measure × month × scope), `GET /api/quality/history`, an as-of backfill CLI, and a "Quality over time (source of truth)" card on `/programs/[measureId]`.
  - **E5–E7:** multi-channel outreach + bulk campaigns (`/campaigns`), ACIP immunization forecasting, and advisory order proposals (`/orders`).
  - **E12 PR-1/PR-2b — pluggable data ingress + WebChart→FHIR adapter core:** DB-less `evaluateBundle`/`evaluateBatch`, a `PatientDataSource` port, and a transport-agnostic WebChart terminology/normalization adapter (HTTP transport deferred to the MIE API contract).
- **UX-8 — monthly program-card trends:** the `/programs` per-card trend reads the monthly `quality_snapshots` series (opt-in `?granularity=month`), telling the real quality-over-time story instead of a flat per-run line.
- **UX-11 — compliance roster mobile cards:** `/compliance` renders per-employee cards on phones (below `md`) instead of a wide horizontally-scrolling table.
- **Accessibility (WCAG 2.2 AA pass):** a code-level audit + remediation across the whole dashboard — a real ARIA combobox for OSHA-reference authoring, `aria-live`/`role=alert` status/error regions, contrast-token fixes, focus rings, target sizes, ARIA tab patterns, and screen-reader data-table alternatives beside every chart. Audit: `docs/WCAG_AUDIT_2026-07-03.md`.
- **Performance (#233):** the two hottest aggregate reads (`/api/compliance/roster`, `/api/hierarchy/rollup`) are ~5–6× faster warm on the live stack — an index-driven `outcomes` scan (3.2s → 41ms), a memoized scale-tenant aggregation, and a per-immutable-run roster cell cache (guarded by a `/evaluate`-on-terminal-run 409).
