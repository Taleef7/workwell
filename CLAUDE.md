...
**Latest (2026-06-26): the post-demo WebChart-convergence roadmap (board #7) is well underway — E10 + all of E11 are merged + live; E11.3 PR-2 (Configure Groups UI) is BUILT and open as PR #206 on `feat/e11-3-segments-ui`, awaiting review/merge (not yet merged).** The June-15 demo produced a roadmap of epics E10–E15 (#182–#187, label `webchart-convergence`). Shipped + deployed since:
...
- **E11.3 PR-2 — Configure Groups UI (BUILT — PR #206, on `feat/e11-3-segments-ui`, awaiting review/merge):** the new backend endpoint `POST /api/segments/preview` (ADMIN, read-only dry-run cohort membership for an unsaved rule; reuses `matchesCohort`/`validateRule`; shared `previewResponse` helper) + a `/admin → Groups` editor (`SegmentsAdmin`/`SegmentsList`/`SegmentEditorModal` — rule builder + applicable measures + INCLUDE/EXCLUDE overrides via the existing `/api/employees/search` + live preview; ADMIN-gated New/Edit/Delete via `canManageSegments`) + the roster `NOT_APPLICABLE` chip (`lib/status.ts`) + the `/compliance` segment filter (`?segment=`). No schema, no new deps. Verified green (backend 704 pass/1 pg-skip; frontend lint clean + 97 vitest pass + build). Codex P2 (trim scalar rule values before save) addressed. Spec/plan are historical context: `docs/superpowers/specs/2026-06-25-e11-3-segments-ui-design.md`, `docs/superpowers/plans/2026-06-25-e11-3-segments-ui.md`. Closes E11 on merge. **Next epics after E11:** E12 #184 (pluggable data adapters), E13–E15.
