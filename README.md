...
| ![Integration and scheduler admin](docs/assets/07-admin.png) **Admin** — integration settings, the nightly scheduler, email provider, and brand switching; every setting is an audited write. | ![Cross-system identity review](docs/assets/08-people.png) **People** — cross-system identity: potential duplicates are surfaced for human review, never auto-merged. |
