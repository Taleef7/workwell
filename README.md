...
- **E15 PR-2 — identity reconcile write path (branch `feat/e15-identity-reconcile`, in review).** The confirm/unlink half of cross-system identity (ADR-022). The first E15 schema — an owner-approved `person_links` table (floor + ceiling, `workwell_spike`; self-creating DDL) recording human-confirmed CONFIRMED/BROKEN assertions between two source records (pair-normalized, direction-independent; `PersonLinkStore` port, store-contract tested). `resolvePeople` becomes override-aware (union-find: CONFIRMED unions records even without a shared identifier, BROKEN splits a bad auto-match); component `personId` is the smallest record ref-key. `POST /api/identity/people/:personId/reconcile` (`{action, tenantId, externalId}`) — CASE_MANAGER/ADMIN-gated + audited (`IDENTITY_LINK_CONFIRMED`/`IDENTITY_LINK_BROKEN`); frontend adds a CM/ADMIN "unlink" action on the person view. All `/api/identity/**` reads are CM/ADMIN too (the directory exposes national/MRN ids + DOB). Descriptive only (ADR-008); E13 reconciliation unaffected; reversible (`DELETE FROM person_links`). 840 backend tests (839 pass / 1 pg-skip); frontend lint + build green. No new deps. See `docs/JOURNAL.md`.
