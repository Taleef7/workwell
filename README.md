...
- **De-Java re-platform underway** (issue `#96`, ADR-008): the backend is being ported off Java/Spring Boot onto a TypeScript / `@mieweb/cloud` stack (`backend-ts/`), strangler-fig behind the **unchanged** Next.js fetch contract, CQL Path C (build-time CQL→ELM, JVM-free Node execution). Merged so far: the engine (#106, no-JVM ELM Explorer in `backend-ts` — surfaces on the deployed stack after the #109 cutover; the current Java-backed `twh.os.mieweb.org` CQL tab has compile + SQL-analogy panels, not the ELM/AST view), auth/CORS (#105), the SQLite-floor + Postgres-ceiling storage contract (#104), and the **Phase-4/4b API strangler** — runs (incl. ALL_PROGRAMS/SITE async via `ctx.waitUntil`), cases, measures (catalog + detail + create/lifecycle + **Spec/CQL/Tests authoring writes**), programs, exports, admin (reads + toggles + write CRUD), AI surfaces, and the 13 read-only MCP tools (all implemented). ~362 `backend-ts` tests green. Auditor packets (run + measure-version + case), evidence upload/download, scheduled appointments, manual case resolve, MAT export, measure analytics (traceability + data-readiness + impact-preview), value-set governance (registry + links + resolve-check/diff/detail + terminology mappings), and admin write CRUD (outreach-template create/update + non-prod demo-reset + waivers list/grant) are all done. **Phase 4b (#108) is complete.** The remaining phase is the **#109 deploy cutover**, now underway on the **Neon/Postgres fallback path** (single container + the existing Neon, no companion services): **PR1** (container entrypoint + image) is merged (#155); the **store-selection seam** (PR #156) makes the TS backend select the SQLite floor or the Pg ceiling by `DATABASE_URL` — proven end-to-end on real Postgres (60 measures, a run → 100 outcomes / 22 cases; 422 `backend-ts` tests green on SQLite). Remaining: evidence `BUCKET`, the shadow deploy, the blue-green flip, JVM retirement. Plan + resume guide: `docs/superpowers/plans/2026-06-15-issue-109-deploy-cutover.md`; narrative in `docs/JOURNAL.md`.
