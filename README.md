...
- **E11.3 PR-2 — Configure Groups UI (spec + plan ready; NOT yet built).** On branch `feat/e11-3-segments-ui`: a `/admin → Groups` editor (rule builder + applicable measures + INCLUDE/EXCLUDE overrides + live membership preview) + a `POST /api/segments/preview` endpoint + the roster `NOT_APPLICABLE` chip and segment filter. Closes E11. Design + plan in `docs/superpowers/`.
