...
| Brand font (Jost) | App keeps Geist (globals' unlayered `body` rule beats the brand's `layer(theme)` one). Since #453 Jost is self-hosted: `sync:brands` rewrites the brand css's Google Fonts `@import` to `@font-face` over `public/fonts/jost-*.woff2`, so the runtime brand switcher no longer fetches from Google | `scripts/copy-brand-css.mjs`, `public/fonts/` |
